...
public class DefaultResultSetHandler implements ResultSetHandler {
  ...
  public List<Object> handleResultSets(Statement stmt) throws SQLException {
    ...
    //定义变量resultMapCount，它表示一个MS对象中resultMap的个数。通常它是1。我们常用的情况也是1。
    // 要注意，第一部分定义的resultSetCount变量和resultMapCount并不一定相等。
    // 比如PreparedStatement对象中有两个结果集——那么此时的resultSetCount就是2.但是xml中select标签的resultMap属性只定义了一个映射——那么此时的resultMapCount就是1
    ...
    //设置嵌套对象的子属性（目测resultSets只给子属性使用的）
  ...
  public void handleRowValues(ResultSetWrapper rsw, ResultMap resultMap, ResultHandler<?> resultHandler,
    ...
    //存在嵌套查询
    //这取决于xml文件中resultMap标签是否定义了collection和association标签
  ...
  private void callResultHandler(ResultHandler<?> resultHandler, DefaultResultContext<Object> resultContext,
      Object rowValue) {
    //存储getRowValue方法解析的结果对象
    resultContext.nextResultObject(rowValue);
    //把单行记录的解析对象存入到ResultHandler的集合当中
  ...
  private void skipRows(ResultSet rs, RowBounds rowBounds) throws SQLException {
    ...
    // 指定数据库游标的移动方向是向前，不允许向后移动，即只能使用ResultSet 接口的next()方法而不能使用previous()方法，否则会产生错误。（默认）
  ...
  private Object getRowValue(ResultSetWrapper rsw, ResultMap resultMap, String columnPrefix) throws SQLException {
    ...
    if (rowValue != null && !hasTypeHandlerForResultObject(rsw, resultMap.getType())) {
      //非!(目测type是基本类型时，才会进入此，且返回结果只取第一列)
  ...
  private Object createResultObject(ResultSetWrapper rsw, ResultMap resultMap, ResultLoaderMap lazyLoader,
    ...
    if (resultObject != null && !hasTypeHandlerForResultObject(rsw, resultMap.getType())) {
      //非!(目测type是基本类型时，才会进入此，且返回结果只取第一列)-->【不是单列的基本类型】
      //获取<resultMap>--><result>/<id>
      final List<ResultMapping> propertyMappings = resultMap.getPropertyResultMappings();
      for (ResultMapping propertyMapping : propertyMappings) {
  ...
  private Object createResultObject(ResultSetWrapper rsw, ResultMap resultMap, List<Class<?>> constructorArgTypes,
    ...
    //对应resultMap的type（<resultMap id="BaseResultMap" type="club.emperorws.entities.Student">）
    ...
    if (hasTypeHandlerForResultObject(rsw, resultType)) {
      //目测type是基本类型时，才会进入此，且返回结果只取第一列-->可以直接获取对象值
      return createPrimitiveResultObject(rsw, resultMap, columnPrefix);
    }
    if (!constructorMappings.isEmpty()) {
      //带有构造器参数的resultObject生成
      ...
      //是interface接口，或有默认构造器
      return objectFactory.create(resultType);
    } else if (shouldApplyAutomaticMappings(resultMap, false)) {
      //自动映射生成对象，且不嵌套映射，则使用构造器的方式创建对象
  ...
  private Optional<Constructor<?>> findConstructorForAutomapping(final Class<?> resultType, ResultSetWrapper rsw) {
    ...
    //只有一个构造器，直接返回（一般是默认构造器）
    if (constructors.length == 1) {
      return Optional.of(constructors[0]);
    }
    //使用了@AutomapConstructor注解的构造器，有且只能有一个@AutomapConstructor
